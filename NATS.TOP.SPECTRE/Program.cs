...
namespace NatsTopCSharp
{
    class Program
    {
        ...
        static void RenderStats(Engine engine, Stats stats)
        {
            ...
            serverInfo.AppendLine($"[bold]Load:[/] CPU: {varz.CPU:F1}%  Memory: {Psize(false, varz.Mem)}  Slow Consumers: {varz.SlowConsumers}");
            serverInfo.AppendLine($"[bold]In:[/]  Msgs: {Nsize(engine.DisplayRawBytes, varz.InMsgs)}  Bytes: {Psize(engine.DisplayRawBytes, varz.InBytes)}  Msgs/Sec: {stats.Rates?.InMsgsRate:F1}  Bytes/Sec: {Psize(engine.DisplayRawBytes, (long)(stats.Rates?.InBytesRate ?? 0))}");
            serverInfo.AppendLine($"[bold]Out:[/] Msgs: {Nsize(engine.DisplayRawBytes, varz.OutMsgs)}  Bytes: {Psize(engine.DisplayRawBytes, varz.OutBytes)}  Msgs/Sec: {stats.Rates?.OutMsgsRate:F1}  Bytes/Sec: {Psize(engine.DisplayRawBytes, (long)(stats.Rates?.OutBytesRate ?? 0))}");
            ...
            if (stats.Connz?.Conns != null)
            {
                foreach (var conn in stats.Connz.Conns)
                {
                    ...
                    string pending = Nsize(engine.DisplayRawBytes, conn.Pending);
                    string colMsgsTo, colMsgsFrom, colBytesTo, colBytesFrom;

                    if (!engine.ShowRates)
                    {
                        colMsgsTo = Nsize(engine.DisplayRawBytes, conn.OutMsgs);
                        colMsgsFrom = Nsize(engine.DisplayRawBytes, conn.InMsgs);
                        colBytesTo = Psize(engine.DisplayRawBytes, conn.OutBytes);
                        colBytesFrom = Psize(engine.DisplayRawBytes, conn.InBytes);
                    }
                    else
                    {
                        ...
                        colMsgsTo = Nsize(engine.DisplayRawBytes, (long)cr.OutMsgsRate);
                        colMsgsFrom = Nsize(engine.DisplayRawBytes, (long)cr.InMsgsRate);
                        colBytesTo = Psize(engine.DisplayRawBytes, (long)cr.OutBytesRate);
                        colBytesFrom = Psize(engine.DisplayRawBytes, (long)cr.InBytesRate);
                    }

                    string lang = conn.Lang;
                    string version = conn.Version;
                    string uptime = conn.Uptime;
                    string lastActivity = FormatDateTime(conn.LastActivity);
